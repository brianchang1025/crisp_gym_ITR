...
def push_local_dataset(
    local_dataset_path: Path,
    target_repo_id: str,
    private: bool = True,
) -> None:
    """Load a local dataset for validation and push its folder contents to the Hub."""
    logging.info("Loading local dataset from %s", local_dataset_path)
    # Validation step requested by user: ensure local dataset is loadable.
    if len(local_dataset_path.parts) < 2:
        raise ValueError(f"Invalid local dataset path: {local_dataset_path}")
    local_repo_id = "/".join(local_dataset_path.parts[-2:])
    local_root = local_dataset_path.parents[1]
    LeRobotDataset(repo_id=local_repo_id, root=local_root)

    logging.info(
        "Pushing local dataset '%s' to Hub as '%s' (private=%s)",
        local_dataset_path,
        target_repo_id,
        private,
    )
    api = HfApi()
    api.create_repo(repo_id=target_repo_id, repo_type="dataset", private=private, exist_ok=True)
    api.upload_folder(
        repo_id=target_repo_id,
        repo_type="dataset",
        folder_path=str(local_dataset_path),
    )
    logging.info("Push complete: https://huggingface.co/datasets/%s", target_repo_id)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--push-to-hub",
        action="store_true",
        help="After conversion, load local dataset and push it to Hugging Face Hub.",
    )
    parser.add_argument(
        "--target-repo-id",
        type=str,
        default=None,
        help="Destination Hub dataset repo id when --push-to-hub is set.",
    )
    parser.add_argument(
        "--public",
        action="store_true",
        help="Push destination repo as public (default is private).",
    )
...
if __name__ == "__main__":
    init_logging()
    args = parse_args()
    converted_root = convert_dataset(
    ...
    if args.push_to_hub:
        if not args.target_repo_id:
            raise ValueError("--target-repo-id is required when using --push-to-hub")
        push_local_dataset(
            local_dataset_path=converted_root,
